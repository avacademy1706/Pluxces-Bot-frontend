
import React, { useState, useRef, useEffect } from "react";
import axios from "axios";
import logo from "./assets/Plexus.png";

/* 🔥 CHATGPT-STYLE FORMATTER */
const formatChatGPTStyle = (text) => {
  if (!text) return "";

  let formatted = text;

  // **bold** → strong
  formatted = formatted.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>");

  // Bullet points (- item)
  formatted = formatted.replace(
    /^- (.*)$/gm,
    `<div style="margin-left:12px">• $1</div>`
  );

  // Paragraph spacing
  formatted = formatted.replace(/\n{2,}/g, "<br/><br/>");
  formatted = formatted.replace(/\n/g, "<br/>");

  return formatted;
};

export default function App() {
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState([]);
  const messagesEndRef = useRef(null);

  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);

  const sendMessage = async () => {
    if (!input.trim()) return;

    const userMsg = {
      from: "user",
      text: input,
      time: new Date().toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      }),
    };

    setMessages((prev) => [...prev, userMsg]);
    setInput("");

    try {
      const res = await axios.post("https://pluxces-bot-backend.onrender.com/chat", {
        message: input,
      });

      const botMsg = {
        from: "bot",
        text: res.data.reply,
        time: new Date().toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
      };

      setMessages((prev) => [...prev, botMsg]);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        {
          from: "bot",
          text: "Server error! Please try again.",
          time: new Date().toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
        },
      ]);
      console.error(err);
    }
  };

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gradient-to-r from-orange-300 via-orange-400 to-orange-500">
      <div className="w-full max-w-md h-full bg-white shadow-2xl flex flex-col overflow-hidden rounded-none">

        {/* HEADER */}
        <div className="bg-gradient-to-r from-orange-500 via-orange-600 to-orange-700 text-white p-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <img
              src={logo}
              alt="Chatbot Logo"
              className="w-10 h-10 rounded-full object-cover bg-white p-1"
            />
            <div>
              <div className="font-bold">Plexus Digitals Bot</div>
              <div className="text-sm text-orange-100">online</div>
            </div>
          </div>
          <div className="text-xl cursor-pointer">⋮</div>
        </div>

        {/* MESSAGES */}
        <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-gray-100">
          {messages.map((msg, idx) => (
            <div
              key={idx}
              className={`flex ${
                msg.from === "user" ? "justify-end" : "justify-start"
              }`}
            >
              <div
                className={`max-w-[75%] px-4 py-2 rounded-xl text-sm break-words shadow
                  ${
                    msg.from === "user"
                      ? "bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-br-none"
                      : "bg-white text-gray-800 rounded-bl-none"
                  }`}
              >
                {msg.from === "bot" ? (
                  <div
                    className="leading-relaxed"
                    dangerouslySetInnerHTML={{
                      __html: formatChatGPTStyle(msg.text),
                    }}
                  />
                ) : (
                  <span>{msg.text}</span>
                )}

                <div className="text-[10px] text-gray-300 text-right mt-1">
                  {msg.time}
                </div>
              </div>
            </div>
          ))}
          <div ref={messagesEndRef} />
        </div>

        {/* INPUT */}
        <div className="p-4 flex gap-2 border-t border-gray-300 flex-shrink-0 bg-white">
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && sendMessage()}
            placeholder="Type a message"
            className="flex-1 px-4 py-2 rounded-full border outline-none focus:ring-2 focus:ring-orange-500"
          />
          <button
            onClick={sendMessage}
            className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 py-2 rounded-full hover:from-orange-600 hover:to-orange-700 transition"
          >
            Send
          </button>
        </div>

      </div>
    </div>
  );
}
